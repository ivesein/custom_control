    ...
    KDApi.loadFile("./css/element.css", model.schemaId, function () {
      KDApi.loadFile("./css/main.css", model.schemaId, function () {
        KDApi.loadFile("./js/vue.min.js", model.schemaId, function () {
          KDApi.loadFile("./js/mytable.js", model.schemaId, function () {
                  ...
                  model.qualityPlanVue = new Vue({
                    ...
                    methods: {
                        ...
                        if (changeData.setRole) {
                          originData.forEach((ov) => {
                            if (changeData.setRole.qp_task_id === ov.id) {
                              ...
                              ov.qp_owner_role =
                                changeData.setRole.qp_owner_role;
                            }
                          });
                        }
                        // 处理setPerson
                      ...
                      itemChange(row) {
                        ...
                        // 获取该节点所在所有父节点
                        let parentIds = this.findParentIds(
                          flatData,
                          row.parent
                        );
