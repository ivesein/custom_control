...
new Vue({
  ...
  methods: {
     ...
     * @Author: zhang fq
     * @Date: 2020-08-26
     * @Description: 处理时间标尺单位切换 目前分为 日 和 时
     */
    handleScaleClicked() {
      this.loading = true;
      if (this.scale < 2) {
      ...
      this.init({ result: layoutData, aoan: aoanInstance }, this.scale);
    },
    handleChange(val) {
      console.log(val);
    ...
    zoomed(d) {
      ...
      if (wh) {
        this.svgInstance
          .attr("width", wh["width"] * k + 20 + "px")
          .attr("height", wh["height"] * k + 20 + "px");
        // this.svgInstance
        //   .attr("transform", function () {
        //     // return " scale(" + k + ")";
        //     return "translate(" + 0 + "," + 0 + ") scale(" + k + ")";
    ...
    init(res, scale = 1) {
      ...
      // 获取项目的开始日期 结束日期
      const START = res.result.option["projectStartTime"];
      const END = res.result.option["projectEndTime"];
      // 整个画布绘制完毕后 获取画布的实际宽高 赋值给画布容器的宽高
      setTimeout(() => {
        let wh = getGroupWH("root-group");
        if (wh) {
          this.svgInstance
            .attr("width", wh["width"] + 20 + "px")
            .attr("height", wh["height"] + 20 + "px");
        }
        this.loading = false;
      }, 100);
      // 从杨勇算法回调中获取数据
      console.log("aoan>>>>>>>>>>>>>", res.aoan);
      let last_node = res.aoan.lastNode;
      let node_data = _.cloneDeep(res.result.nodeCoordinate); // 获取节点数据
      let linkData = []; // 储存边数据
      let allData = _.cloneDeep(res.result.edges); // 获取所有与边相关的数据
      ...
      taskInfo.forEach((v) => {
        ...
        if (v.link) {
          if (v.link.lines && v.link.lines.length > 0) {
            // 修复连接线只有一条且为垂直方向时的 任务名lable显示坐标计算错误bug
            if (v.link.lines.length === 1 && v.link.lines[0].direction === 2) {
              text.x = v.link.lines[0].start.x;
              text.y =
                (v.link.lines[0].end.y - v.link.lines[0].start.y) / 2 +
                v.link.lines[0].start.y;
              text.ifRotate = true;
            } else {
              for (let i = 0; i < v.link.lines.length; i++) {
                if (v.link.lines[i].direction === 1) {
      ...
      // 创建缩放对象
      let zoom = d3.zoom().scaleExtent([0.5, 2]).on("zoom", this.zoomed);
      // .on("end", zoomended);
      ...
      // 创建根组
      this.rootG = this.svgInstance.append("g").attr("id", "root-group");

      // 绘制标尺
      const role_height = 40;
      const role_title_width = marginLeft - 5;
      let roleGroup = this.rootG.append("g").attr("css", "role-group");
      let role_offset = 5;
      let role_dy = 30;
      let roleHeader = [
        {
      ...
      roleHeader.forEach((v, k) => {
        let roleCell = roleGroup.append("g").attr("css", "role-cell");
        roleCell
          .append("rect")
          .attr("x", function (d) {
            return role_offset;
          ...
          .attr("y", function (d) {
            return role_offset + role_height * k;
          })
      ...
      // console.log("END>>>>>>>>>>>>>>>>>>", END);
      // 计算绘制标尺所需年月 日期数据
      this.dateData = getdiffdate(START.split(" ")[0], END.split(" ")[0]);
      ...
      linkData.forEach((link, k) => {
        if (link.lines) {
          if (link.intersections && link.intersections.length > 0) {
            link.intersections.forEach((li) => {
              link.lines.forEach((ll) => {
                ...
                if (li.line.direction === 1) {
                  // 被穿线如果是水平的 那么穿线就是垂直的 只有垂直方向会有偏移量
                  // 所以穿点坐标只考虑 x加上穿线（垂直线）的x偏移量
                  if (
                    ll.start.y <= li.location.y &&
                    li.location.y <= ll.end.y
                  ) {
                    x =
                      (offsetX + xUnit) * li.location.x +
                      ll.start.xOffset * LINE_OFFSETX_UNIT +
                      marginLeft;
                    // 换算 切换到小时后 过桥半圆的坐标
                    if (scale === 2) {
                      let locationx = conversionCoordinates(li.location.x);
                      x =
                        (offsetX + xUnit) * locationx.int * 24 +
                        (offsetX + xUnit) * locationx.double +
                        ll.start.xOffset * LINE_OFFSETX_UNIT +
                        marginLeft;
                    }
                    li.location.xOffset = ll.start.xOffset; //将坐标的偏移量挂载到穿点坐标的xOffset属性上
                    y =
                      (offsetY + yUnit) * li.location.y +
                      li.line.start.yOffset * LINE_OFFSETX_UNIT +
                      marginTop;
                  }
                } else if (li.line.direction === 2) {
                  // 被穿线如果是垂直   x 直接加上被穿线的x偏移量
                  if (
                    ll.start.x <= li.location.x &&
                    li.location.x <= ll.end.x
                  ) {
                    x =
                      (offsetX + xUnit) * li.location.x +
                      li.line.start.xOffset * LINE_OFFSETX_UNIT +
                      marginLeft;
                    if (scale === 2) {
                      let locationx = conversionCoordinates(li.location.x);
                      x =
                    ...
                    li.location.xOffset = li.line.start.xOffset; //将坐标的偏移量挂载到穿点坐标的xOffset属性上
                    y =
                      (offsetY + yUnit) * li.location.y +
                      ll.start.yOffset * LINE_OFFSETX_UNIT +
                      marginTop;
                  }
                }

                if (x === 0 && y === 0) return; //x=0 y=0 即没有找到穿点所在线段 不用绘制 修复部分半圆桥会绘制的原点的bug
                this.rootG
                  .append("path")
                  .attr("d", function () {
                    return drawBridgeArc(x, y, BRIDGE_R, direction);
                  })
                  .attr("stroke", function (d) {
                    if (li.critical) return "red";
                    if (li.line.shape === 3) return LINE_WAVE_COLOR;
                    return li.dummy ? LINE_DASHED_COLOR : LINE_DEFAULT_COLOR;
                  })
                  .attr("stroke-dasharray", function (d) {
                    return li.dummy ? STROKE_DASHARRAY : "0,0";
                  })
                  .attr("fill", "none");
              });
              xPoints.push(li);
              ...
              let flag = intersection.find((ai) => {
                return (
                  ai.line.start.x === li.line.start.x &&
                  ai.line.start.y === li.line.start.y &&
                  ai.line.end.x === li.line.end.x &&
                  ai.line.end.y === li.line.end.y
                );
              });
              if (flag) {
                flag.xPoints.push(li.location);
      ...
      linkData.forEach((link) => {
        if (link.lines) {
          ...
          link.lines.forEach((line, index) => {
            let flag = intersection.find((item) => {
              return (
                line.start.x === item.line.start.x &&
                line.start.y === item.line.start.y &&
                line.end.x === item.line.end.x &&
                line.end.y === item.line.end.y
              );
            });
            if (flag) {
              // 如果本次循环的线段上有交叉点 进行处理
              ...
              // 循环处理该被穿线上的每一个交叉点 截取
              flag.xPoints.forEach((xpoint, xindex) => {
                // 如果是第一次截取 直接截取
                if (xindex === 0) {
                  newLines.push(...lineSplit(line, xpoint, scale)); // 将第一次截断的线段存入到线段数组中
                } else {
                  // let lindex=0
                  // 如果不是第一次截取  要先判断当前交叉点在已经被截断的哪一段上 继续截取
                  for (let i = 0; i < newLines.length; i++) {
                    // 找到当前循环到的交叉点所在的被截断后的那条线段  继续截
                    if (onTheLine(newLines[i].start, newLines[i].end, xpoint)) {
                      // 删除原来的线段 插入截取后的线段
                      newLines.splice(
                        i,
                        1,
                        ...lineSplit(newLines[i], xpoint, scale)
                      );
                      break; // 找到后处理完就跳出循环
                    }
                  }
 ...
 * @Author: zhang fq
 * @Date: 2020-08-20
 * @Description: 修复波浪线带箭头时 线段起始对不上箭头的尾部的bug
 */
 ...
 * @Author: zhang fq
 * @Date: 2020-08-20
 * @Description: 修复 优化波浪线与箭头连接点坐标后出现的箭头位置不对的bug
 */
 ...
 * @Author: zhang fq
 * @Date: 2020-08-26
 * @Description: 计算切换到小时后处理天坐标和小时坐标的算法
 */
