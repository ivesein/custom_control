...
class Graph extends Evee {
  ...
  _addVertexsMap(v) {
    let oldCritical = this.vertexsMap[v.code]
      ? this.vertexsMap[v.code].critical
      : false;
    if (oldCritical === true) {
      v.critical = true;
    }
  ...
  _rawTasksHandle(allTasks) {
    ...
    for (let i = 0; i < allTasks.length; i++) {
      ...
      taskInfo.isCriticalTask = taskInfo.isCriticalTask
        ? taskInfo.isCriticalTask === "true" || taskInfo.isCriticalTask === true
          ? true
          : false
        : false;
  ...
  _registerLink(link) {
    ...
    if (lines.length > 0) {
      for (let i = 0; i < lines.length; i++) {
        ...
        } else if (line.direction === _LINE_DIRECTION.HORIZON) {
          // 水平线
            ...
            for (let i = 0; i < edge.link.lines.length; i++) {
              if (edge.link.lines[i].start.y !== this.nodes[endCode].y) {
                ...
                if (edge.link.lines[i].start.y > edge.link.lines[i].end.y) {
                  ...
                  edge.link.lines[i].start.y < edge.link.lines[i].end.y
                ) {
                  edge.link.lines[i].end.y += 1;
                }
              }
                ...
                if (this.aoan.edgesMap[keys[i]].link.type !== 1) {
                  ...
                  for (let i = 0; i < edge.link.lines.length; i++) {
                    if (
                      ...
                      if (this.contactPoint[startCode].top.length > 0) {
                        // 开始节点上方的连线
                        for (
                          let j = 0;
                          j < this.contactPoint[startCode].top.length;
                          j++
                        ) {
                          for (
                            let k = 0;
                            k <
                            this.contactPoint[startCode].top[j].link.lines
                              .length;
                            k++
                          ) {
                            let a = this.contactPoint[startCode].top[j].link
                              .lines[k].start.y;
                            let b = this.contactPoint[startCode].top[j].link
                              .lines[k].end.y;
                            // 节点上方 y 大的 -1
                            if (a > b) {
                              this.contactPoint[startCode].top[j].link.lines[
                      ...
                      if (this.contactPoint[startCode].bottom.length > 0) {
                        // 开始节点下方的连线
                        for (
                          let m = 0;
                          m < this.contactPoint[startCode].bottom.length;
                          m++
                        ) {
                          for (
                            let n = 0;
                            n <
                            this.contactPoint[startCode].bottom[m].link.lines
                              .length;
                            n++
                          ) {
                            let a = this.contactPoint[startCode].bottom[m].link
                              .lines[n].start.y;
                            let b = this.contactPoint[startCode].bottom[m].link
                              .lines[n].end.y;
                            // 节点下方 y 小的 -1
                            if (a > b) {
                              this.contactPoint[startCode].bottom[m].link.lines[
    ...
    this.viz.getSvgData(initData).then(function (element) {
      ...
      for (let i = 0; i < nodes.length; i++) {
        __nodes[nodes[i]].critical = that.aoan.vertexsMap[nodes[i]]
          ? that.aoan.vertexsMap[nodes[i]].critical
            ? that.aoan.vertexsMap[nodes[i]].critical
            : false
          : false;
...
class ActivityOnArrowNetwork extends Graph {
  ...
  layoutPlan(callback) {
    ...
    // })
    let factor = this.layoutFactor;
    let viz = new Viz({ workerURL: "../js/full.render.js" });
